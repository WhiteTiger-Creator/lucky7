...
> **Governance decision (2026-05-25 - #DB-5358)** Nadia: trust contention (final). Each origin's `trust_exposure_score` is computed independently, so a target env reachable from several origins is currently counted by every one of them. A target reachable from MORE THAN ONE origin is charged to exactly ONE of them. For each such contended target, take the origins that reach it (its claimants) and pick the OWNER: the claimant whose strongest bounded simple-path score TO THAT TARGET is highest, ties broken by lexicographically smallest origin name. Note this is the per-target path score, NOT the origin's overall `trust_exposure_score` -- the two pick different owners. The owner adds that same per-target score to its own `trust_exposure_score`; every other claimant adds nothing for that target, and a target reachable from exactly one origin is not contended and adds nothing to anybody. The bonus is folded in BEFORE `trust_path_digest` is taken, so the digest commits to the attributed score. `trust_reachable_envs` and `trust_strongest_path` are unaffected: attribution changes what an origin SCORES, never what it reaches. Worked example: target t is reached by origins a (path score 9) and b (path score 4); a owns t and adds 9 to its own score, b adds 0, and t still appears in both origins' `trust_reachable_envs`. Interaction with #DB-5319, stated explicitly so it cannot be misread: this contention credit is ADDED ON TOP OF the origin's base node-disjoint packing from #DB-5319 — it does NOT replace it. Every reachable env already contributes through that base packing; contention only adds an extra per-target credit for the contended targets an origin owns, and a non-contended target adds nothing beyond whatever it already contributed to the base packing (it is never forced to zero). So `trust_exposure_score = base_node_disjoint_packing + sum(per_target_score for each contended target this origin owns)`. Combined worked example: if origin a's base node-disjoint packing over its 1-to-3-edge paths is 20, and a additionally owns two contended targets with per-target path scores 9 and 5, then a's `trust_exposure_score` is 20 + 9 + 5 = 34; an origin that owns no contended target keeps exactly its base packing. Contention is strictly BETWEEN DIFFERENT origin envs — it never arises within a single origin. Multiple paths from the SAME origin to the same target are NOT contention: a single origin's own paths are already resolved by the node-disjoint packing of #DB-5319, which counts at most one path through any shared env, so same-origin multi-path never adds a contention credit and never turns the base packing into a per-target sum. Negative worked example (the common misread, ruled out): if origin a reaches target t by two different simple paths a>x>t (score 9) and a>y>t (score 7), that is entirely a's own packing question — t is not contended, a adds nothing for it beyond the packing, and a's score is NOT 9+7. A target t becomes contended only when a SECOND, different origin (say b) also reaches it; only then is there an owner and a contention credit. So on a graph with one origin there is no contention at all, and the exposure score is exactly the node-disjoint packing.
...
> **Governance decision (2026-05-31 - #DB-5366)** Yusuf: risk-ledger carry-out backlog term, final. The flat `rotation_segment_count*15 + defer_segment_count*10` backlog bonus recorded in #DB-5313 — and reaffirmed as composition-unchanged in #DB-5360 — weighted raw segment counts rather than the time actually spent under rotation/defer coverage, so a window split into many tiny segments was over-credited while one long segment was under-credited. The backlog bonus is now computed from the post-attenuation overlap DURATIONS and ROUNDS UP: `carry_out_ms = min(carry_in_ms + actionable_duration_ms + ceil(rotation_overlap_ms / 7) + ceil(defer_overlap_ms / 9), 921)`, taking each window's final `rotation_overlap_ms` (after the #DB-5354 reopen-precedence subtraction) and `defer_overlap_ms`. In integer arithmetic ceil(x/n) is -(-x // n). This is final and revises the carry-out composition recorded in #DB-5313 on that one point only: the carry-in formula, the idle-gap decay, the ledger-adjusted credit and the 921 ms carry-out cap from #DB-5346 are all unchanged, and `rotation_segment_count`/`defer_segment_count` keep their #DB-5364 definitions everywhere else they appear (queue_hash fields, volatility scoring, the zero-rotation priority checks). The #DB-5313 and #DB-5314 worked examples predate this correction and use the flat segment weights, so their carry-out figures are stale on this point. ROUNDING: rotation_overlap_ms // 7 = CEIL. ROUNDING: defer_overlap_ms // 9 = CEIL.

> **Governance decision (2026-06-01 - #DB-5368)** Yusuf: ledger-pressure trust-exposure amplifier, final. A window's containment backlog does not depend on its own volume alone: a window sitting on a high blast-radius replica env compounds the responder cost, so the ledger pressure now folds in the window's attributed `trust_exposure_score` — the #DB-5319 node-disjoint packing PLUS the #DB-5358 contention credit, i.e. the exact value emitted in `trust_exposure_score`, not the bare packing — divided by 11 and FLOORED: `ledger_pressure_score = ceil(carry_out_ms / 80) + ceil(carry_in_ms / 120) + max(alert_count - 1, 0) + (trust_exposure_score // 11)`. The three earlier terms and their rounding (the 80 carry-out ceiling from #DB-5362, the 120 carry-in ceiling from #DB-5338, the alert term) are unchanged; this only appends the trust term. Because the amplifier reads the attributed exposure, an error in the trust packing or its contention attribution propagates into the ledger pressure and thus the priority tier. ROUNDING: trust_exposure_score // 11 = FLOOR.
Reviewers should reconcile behavior questions against #DB governance decisions rather than chat excerpts.
